...
predictions = {
    ...
    # 'SOPH🦈':     [ 5,18,24, 7, 2,23,21,16, 9,20, 3,14, 1,22, 4,19, 6,17, 8,25,10,15,11,13,12, 26],
    # 'ALIZ🦎':     [20,21,22,23,17,24, 2,18, 1, 8, 6, 5,25,16,12,13, 7,15, 4,14,11,10, 3,19, 9, 26],
    # 'HETH🍣':     [24,22,20,16, 7,25, 2,19, 6,13,15, 8,23,18,21, 5, 3,10,11,17,14, 1, 4, 9,12, 26],
    # 'MATT🍁':     [19,21, 6,18, 5,23,24, 7,25, 8, 2, 9,22,12,17,15,20,16,14,10, 3, 1,13,11, 4, 26],
    # 'DICE🎲':     [ 9,25, 4,24,19,10, 5,14,17,22, 6,11,21,15, 7, 1,23,12,18, 2, 8,20,16, 3,13, 26],
    # 'ALEX👽':     [18,25,22,24,16,13,19, 9,11, 4, 3, 8, 6,20, 1, 7,12,15,14,10,23, 5,17, 2,21, 26],
    # 'WINZ🪅':     [24,16,14,10, 1,19, 2,12,22,11,21, 5,25,13,20, 3, 8,23, 9,17,15, 6, 7,18, 4, 26],
    # 'STEN🍸':     [23, 8,17,11,25, 4, 2, 3,15,13,22, 9,20, 5,21,14, 6,19,12,18,24, 1, 7,16,10, 26],
    # 'PETE🛸':     [25,18,17, 2,24, 7,23, 8,22,13,19,16,15,12,11,14,10, 6, 5, 9,21, 1, 3,20, 4, 26],
    # 'NICK👺':     [24,20, 6,12,11,23,10,14, 2,21, 3,22, 7,15, 4,25,16, 9, 8,17,19,13, 5,18, 1, 26],
    # 'IVYY🦄':     [23,22,15, 6,13,24, 1,11,21,25, 7, 5,19,12, 4,16, 8,10, 3, 9,14, 2,18,20,17, 26],
    # 'RACH🎭':     [22,21,20, 3, 2, 9, 1, 4,25,13,17,18,19,12,24,23, 5,16, 6,10, 7, 8,15,11,14, 26],
    # 'LIND🍄':     [23, 7,18,15,11,25,12,21,24,22, 4,10, 5,20,19, 3, 8,14, 1, 6, 9,17, 2,16,13, 26],
    # 'ELEN🪆':     [25,22,15,17, 8,14, 9,13, 7,20, 3, 4,18,12,24, 6,19,16,10,11,23, 2, 1,21, 5, 26],
    # 'ALSH🛼':     [21,23,12,13,11,14, 9,10, 3, 7,19,18,15,16, 1,22, 6,20, 2, 4,17,24, 5, 8,25, 26],
    # 'ZYLA🦓':     [ 6,15, 3, 9,19,18,20,17, 7, 4,12,14,10, 1,23,21,16, 5, 8,24, 2,25,13,11,22, 26],
    # 'MIKE🥃':     [25, 3,22, 7,24,21,20, 9,19,13,12, 2,16,10,23, 4,18, 8, 5,14, 6,15, 1,17,11, 26],
    'YAIR👨‍🍳':     [25,24,17, 7, 3,16, 1, 6,21,10,18, 4,22,11,23,15, 8,20, 2, 9,13,12,19,14, 5, 26],

    'CJCJ✨':   [10, 9, 8, 4,17, 5, 2,23,22,15,14,18, 1,20,12,25,19, 3,16,11, 6,24, 7,13,21, 26],
}
# fmt: on
...
def validate_predictions():
    for contestant, ordering in predictions.items():
        ...
        expected = collections.Counter(range(1, 27))
        if count_by_number != expected:
            print("predictions are not 1-26 for ", contestant)
            sys.exit()
...
known_outcomes = {
    "A": "m",  # AI-Assisted Year-End Hot 100 Song
    "B": "m",  # Starr Wars: Vita VonTesse Beats Mia on Drag Race
    "C": "m",  # Change in 30 Tallest Skyscrapers
    "D": "m",  # NFL Octuple Scorigami!
    "E": "m",  # Dragon/Wings at 10k+ Drone Show
    "F": "m",  # Dragging Wings at Victorias Secret Fashion Show
    "G": "m",  # Garfield Eats 7 Donuts
    "H": "m",  # Colbert's Final Wave is with Right Hand
    "I": "m",  # 6 Decks of Yellow Cards at World Cup
    "J": "m",  # Grogu Sneezes, Farts, Coughs, or Burps
    "K": "m",  # Yaaasss Queeen captured by the winner of the World Chess Championship
    "L": "m",  # Jake From State Farm Summoned Thrice
    "M": "m",  # PREDICULOUS in Wheel of Fortune Puzzle
    "N": "m",  # 2 Immortals in Combat in Mortal Kombat 2
    "O": "m",  # Boomer Wins A Razzie
    "P": "m",  # $40,000,000,000.00 < National Debt
    "Q": "m",  # (Butt) Crack in the Bridge (-rton)
    "R": "m",  # Nom Nom Nom Nom Nom Nom Nom: No Tony Wins
    "S": "m",  # Forky Gets Stuck
    "T": "m",  # All Cats Lose Anthrocon Dance Competition
    "U": "m",  # 5 Sluots in a Row, Balled on The Wall
    "V": "m",  # Johan Wins An Olympic Medal... Any Johan
    "W": "m",  # Obama's Playlist Features Multiple Sex Acts
    "X": "m",  # Blues Outperform Reds: Midterms or Sports
    "Y": "m",  # 150 Spanks from @thedonutdaddy
    "Z": "m",  # Chaos Erupts! Wait, not Chaos. 50 Volcanoes.
}
